...
export const getPoolBlocks = async (poolId, pages = 5) => {
  // ?page=1&pageSize=2000
  // Get the first(last) 15 blocks and pageCount
  const { result, pageCount } = await sendRequest(
    `/api/v2/pools/${poolId}/blocks`,
    {
      method: "GET",
    },
  );
  ...
  for (let i = 1; i <= pages && pages < pageCount; i += 1) {
    blockPromises.push(
